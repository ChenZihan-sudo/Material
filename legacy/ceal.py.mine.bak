...
class CEALConv(MessagePassing):
    def __init__(
        ...
        **kwargs,
        ...
        out_channels: dimension of node message (after linear transform)
        ...
        # Equations 5~6 in the PNA paper
        ...
        # Pre/Post MLPs
        # * pre_nns: (3*Fin)Linear(Fin)->Relu->(Fin)Linear(Fin)->...->Relu->(Fin)Linear(Fin)
        # * post_nns: (in_chan)Linear(Fout)->Relu->(Fout)Linear(Fout)->...->Relu->(Fout)Linear(Fout)
        # * lin: (out_chan)Linear(out_chan)
        ...
        if self.aggMLP:
            ...
            for aggregator in self.aggregators:
                if aggregator == "sum":
                    ...
                    mean = scatter(inputs, index, 0, None, dim_size, reduce="mean")
                    mean_squares = scatter(inputs * inputs, index, 0, None, dim_size, reduce="mean")
                    out = weights[4] * (mean_squares - mean * mean)
                    if aggregator == "std":
                        out = torch.sqrt(torch.relu(out) + 1e-5)
                else:
                    raise ValueError(f'Unknown aggregator "{aggregator}".')
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.in_channels}, " f"{self.out_channels}, towers={self.towers}, " f"edge_dim={self.edge_dim})"
