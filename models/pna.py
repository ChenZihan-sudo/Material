...
class PNA(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        default_conv = PNAConv(self.conv_in_dim, conv_out_dim, deg=deg, **conv_params)
        # Except for the first conv, in_channels for last convs are out_dim.
        # That is (out_dim)conv(out_dim)
        last_convs = [PNAConv(conv_out_dim, conv_out_dim, deg=deg, **conv_params) for i in range(num_layers - 1)]
